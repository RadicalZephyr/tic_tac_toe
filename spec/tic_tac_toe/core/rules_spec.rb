...
describe TicTacToeRZ::Core::Rules do
  ...
  describe 'when checking if there is a winner' do
    ...
    it 'knows when someone has won' do
      ["X", "O"].each do |mark|
        TicTacToeRZ::Core::Board.win_places.each do |places|
          board = TicTacToeRZ::Core::Board.empty_board
          set_marks(board, mark, places)
          expect(TicTacToeRZ::Core::Rules.who_won?(board)).to eq(TicTacToeRZ::Core::Mark.new(mark))
        end
      end
    end

    it 'returns nil for non-won boards' do
      expect(TicTacToeRZ::Core::Rules.who_won?(board)).to eq(nil)
  ...
  describe 'when checking for draws' do
    it 'knows when its a draw' do
      ...
      set_board(board, ["X", "X", "O",
                        "O", "O", "X",
                        "X", "X", "O"])
      expect(TicTacToeRZ::Core::Rules.draw?(board)).to eq(true)
    ...
    it 'doesn\'t report false positive draws' do
      expect(TicTacToeRZ::Core::Rules.draw?(board)).to eq(false)
    end
  end
  ...
  describe 'when checking if the board is done' do
    ...
    it 'is not complete if it is empty' do
      expect(TicTacToeRZ::Core::Rules.finished?(board)).to eq(false)
    end
    ...
    it 'may be not full if someone won' do
      set_board(board, ["X", "X", "X",
                        "O", "O", " ",
                        "X", "O", " "])
      expect(TicTacToeRZ::Core::Rules.finished?(board)).to eq(true)
    end
